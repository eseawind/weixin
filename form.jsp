...
<html>
<head>
	<title id='product_name'><%=indexName%>-V1.0</title>
...
Ext.onReady(function() {
	...
	var createNode = function(nodeId, pageParam) {
		...
		var hdCall = function(f) {
			...
			//如果不是layout页面，是FormNode页面，则有showPage方法
			...
			if (page.isXType('form')) {
				...
				Jxstar.queryData(nodeId, options);
			}
			//page = null;
		};

		//异步从JS文件加载功能对象
		var pathname = define.formpage;
	...
	var f_success = function(data) {
		...
		var nodeid = '<%=nodeid%>';
		createNode(nodeid, {pageType:'oneform'});
