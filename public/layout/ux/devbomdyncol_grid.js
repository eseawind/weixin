 ...
 */
JxDevDynColBom = function(config){
	//当前功能ID
	//config.curfunid = 'mat_size_det';
	
	//小计列字段名
	//config.sum_colname1 = 'mat_appdet__mat_num';
	
	//小计列字段名
	//config.sum_colname2 = 'mat_appdet__mat_num';
...
JxDevDynColBom.prototype = {	
	//添加20个动态列
	initDynCols: function(cols) {
		var me = this;
		//先找到要添加列的位置，根据小计列字段名
		var index = this.findIndex(cols, me.sum_colname1);
		...
		for (var j = 0; j < me.DYNNUM; j++) {
			...
			var col = {col:{header:j, width:55, sortable:false, align:'left',
						editable:true, hcss:'color:#3039b4;',hidden:true,
			            editor:new Ext.form.TriggerField({
		             	maxLength:20,
						title:'size_num2__'+j,
	...
	saveDynData: function(grid) {
		...
		for (var i = 0; i < mrow.length; i++) {
			...
			var names = me.getColNames(sid, grid,'size_num2__');//取尺码名称
	...
	caluSumData: function(field, record, grid) {
		...
		record.set(me.sum_colname1, sum);
	...
	findIndex: function(cols) {
		...
		for (var i = 0; i < cols.length; i++) {
			var f = cols[i].field;
			if (f && f.name.indexOf(me.sum_colname1) == 0) {
				return i;
			}
		}
		return 0;
	...
	addDynHeader: function(datas, grid) {
		...
		var last = cm.findColumnIndex('size_num__9');
	...
	//显示需要的列，传入尺码值数组
	showMainHeader: function(datas,cdatas, grid,isSizeUsage) {
		var cm = grid.getColumnModel();
		//alert(isSizeUsage);
		//找出第一个尺码列，尺码
			var first = cm.findColumnIndex('size_num__0');
			for (var i = 0; i < datas.cols.length; i++) {
				if(isSizeUsage!='0'){//用量用系统参数opr.dev.bomsizeusage控制是否显示
	...
	//显示动态列的标题
	showMoreHeader: function(headers,cheaders, grid,isSizeUsage) {
		var me = this;
		//把标题最长的一个放在前面
		var newhs = [];
		...
		for (var i = 0; i < newhs.length; i++) {
			if (i == 0) {
				//显示需要的列
				me.showMainHeader(newhs[i],newhc[i], grid,isSizeUsage);
			} else {
	...
	showDynData: function(datas,cdatas,datas2, grid) {
		...
		if (trs && trs.getCount() > 0) {
			...
			for (i = 0; i < tds.getCount(); i++) {
				var cls = tds.item(i).dom.className;
				if (cls.indexOf(me.sum_colname1) >= 0) {
	...
	updateBotValue: function(field, grid) {
		...
		for (var i = 0; i < cnt; i++) {
			...
			var nums = rec.get(me.sum_colname1);
