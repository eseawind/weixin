...
JxDynCol.prototype = {	
	...
	showMainHeader: function(datas, grid) {
		...
		for (var i = 0; i < datas.cols.length; i++) {
			...
			var posi=datas.cols[i].indexOf ('<br>');
			if(posi>0){
				var width=posi*21>50?posi*21:40;
				cm.setColumnWidth(first+i,width);
			}else{
				var width=datas.cols[i].length*15>60?datas.cols[i].length*15:60;
				cm.setColumnWidth(first+i,width);
			}
