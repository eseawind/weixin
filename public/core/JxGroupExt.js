...
(function(){
	...
	init: function(pageNode) {
		...
		this.numFields[b] = ['recordnum', '记录数'];
	},
	...
	saveCase: function() {
		...
		if (self.selStatId.length == 0) {
			Ext.MessageBox.prompt(jx.base.hint, '请输入统计方案的名称', function(btn, text) {
				if (btn != 'ok') return;
				if (text.length == 0) {
					JxHint.alert('必须填写统计方案名称');
					return;
				}
	...
	loadStaCase: function(pageNode, combo) {
		...
		var where_sql = "fun_id = ? and (is_share = ? or user_id = ?)";
