...
(function(){
	...
	renderToolQry: function(nodeg, qrycond) {
		...
		var renderQry = function() {
			...
			grid.qryCt.doLayout();
			if (!Ext.isChrome) grid.setHeight(grid.ownerCt.getHeight());//单执行doLayout无效
	...
	addQryField: function(qrycfg, mycols, qryrow) {
		...
		for (var i = 0, c = 0, n = mycols.length; i < n; i++){
			...
			if (mf.name == qrycfg.colcode) {
				...
				if (noshow == '0' && coltype == 'date'&&qrycfg.condtype!='is null') {
				//添加日期范围查询控件
					qrycfg.colname = str;
