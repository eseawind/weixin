Jxstar.currentPage = function() {
	...
	{col:{header:'开始时间', width:136, sortable:true, renderer:function(value) {
			return value ? value.format('Y-m-d H:i') : '';
		}}, field:{name:'wf_assign__start_date',type:'date'}},
	{col:{header:'受限时间', width:128, sortable:true, renderer:function(value) {
			return value ? value.format('Y-m-d H:i') : '';
	...
	var renderTask = function(val, metaData, record) {
		...
		var html = '<a href="#" style=\'color:#0080FF;\' '+ chgcolor +' onclick="JxUtil.showCheckData(\''+ funId +'\', \''+ dataId +'\', \''+ userId +'\');">'+ val +'</a>';
		return html;
	};
