...
Jxstar.currentPage = function() {
	...
	var getDataUrl = function(){
		...
		var endcall = function(data) {
			...
			var tar_url=setSrcForm.getForm().findField('target_url');
			var tar_username = setSrcForm.getForm().findField('target_username');
			var tar_password = setSrcForm.getForm().findField('target_password');
			tar_url.setValue(Jxstar.systemVar.opr__fun__compare__targer_url);
			tar_username.setValue(Jxstar.systemVar.opr__fun__compare__targer_user);
			tar_password.setValue(Jxstar.systemVar.opr__fun__compare__targer_pwd);
		};
	...
	var expData = function(){
		...
		var options = {
				timeout : 60000 * 10 ,// 10分
				wait : true
			}
		//发送请求
		Request.postRequest(params, endcall,options);
	};
	...
	var expoption = {
		...
		items:[{
			...
			style:'margin:0 0 0 38px;font-size:13px;',
			xtype:'container',
			layout:'absolute',
			width:150,
			height:24,
