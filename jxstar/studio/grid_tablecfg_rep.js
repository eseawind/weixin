Jxstar.currentPage = function() {
	...
	var tablespaceData = Jxstar.findComboData('tablespace');
	var tabletypeData = Jxstar.findComboData('tabletype');
	...
	{col:{header:'*表名称', width:134, sortable:true, editable:true, hcss:'color:#0000ff;',
		editor:new Ext.form.TextField({
			maxLength:50, allowBlank:false
		})}, field:{name:'dm_tablecfg__table_name',type:'string'}},
	{col:{header:'*表标题', width:123, sortable:true, editable:true, hcss:'color:#0000ff;',
		editor:new Ext.form.TextField({
			maxLength:50, allowBlank:false
	...
	{col:{header:'表空间', width:100, sortable:true, defaultval:'jxstar', align:'center',
		...
		editor:new Ext.form.ComboBox({
			...
			displayField: 'text',
			editable:false,
			value: tablespaceData[0][0]
		}),
		renderer:function(value){
			for (var i = 0; i < tablespaceData.length; i++) {
				if (tablespaceData[i][0] == value)
					return tablespaceData[i][1];
			}
		}}, field:{name:'dm_tablecfg__table_space',type:'string'}},
	{col:{header:'*表主键', width:100, sortable:true, editable:true, hcss:'color:#0000ff;',
		editor:new Ext.form.TextField({
			maxLength:50, allowBlank:false
		})}, field:{name:'dm_tablecfg__key_field',type:'string'}},
	{col:{header:'*数据源', width:100, sortable:true, defaultval:'default', editable:true, hcss:'color:#0000ff;',
		editor:new Ext.form.TextField({
	...
	{col:{header:'表类型', width:69, sortable:true, defaultval:'0', align:'center',
		...
		editor:new Ext.form.ComboBox({
			...
			displayField: 'text',
			editable:false,
			value: tabletypeData[0][0]
		}),
		renderer:function(value){
			for (var i = 0; i < tabletypeData.length; i++) {
				if (tabletypeData[i][0] == value)
					return tabletypeData[i][1];
			}
		}}, field:{name:'dm_tablecfg__table_type',type:'string'}},
	{col:{header:'表说明', width:250, sortable:true, editable:true, hcss:'color:#3039b4;',
		editor:new Ext.form.TextField({
			maxLength:200
		})}, field:{name:'dm_tablecfg__table_memo',type:'string'}},
	{col:{header:'表ID', width:100, sortable:true, hidden:true}, field:{name:'dm_tablecfg__table_id',type:'string'}}
	];
	...
	config.eventcfg = {
		...
		compdb: function() {
			...
			var hdcall = function() {
				...
				//发送请求
				Request.postRequest(params, endcall,{timeout:30*60*1000});
			};
