Jxstar.currentPage = function() {
	...
	var cols = [
	{col:{header:'发送时间', width:116, sortable:true, renderer:function(value) {
	...
	{col:{header:'消息发送状态', width:100, sortable:true, align:'center',
		...
		renderer:function(value){
			for (var i = 0; i < notestatusData.length; i++) {
				if (notestatusData[i][0] == value)
					return notestatusData[i][1];
			}
	...
	{col:{header:'发送来源', width:78, sortable:true, align:'center',
		...
		renderer:function(value){
			for (var i = 0; i < notesrcData.length; i++) {
				if (notesrcData[i][0] == value)
					return notesrcData[i][1];
			}
	...
	{col:{header:'来源记录ID', width:100, sortable:true, hidden:true}, field:{name:'sys_note__send_srcid',type:'string'}},
	{col:{header:'数据ID', width:100, sortable:true, hidden:true}, field:{name:'sys_note__data_id',type:'string'}},
	{col:{header:'短信ID', width:100, sortable:true, hidden:true}, field:{name:'sys_note__note_id',type:'string'}},
	{col:{header:'接收用户ID', width:100, sortable:true, hidden:true}, field:{name:'sys_note__user_id',type:'string'}},
	{col:{header:'功能ID', width:100, sortable:true, hidden:true}, field:{name:'sys_note__fun_id',type:'string'}}
	];
