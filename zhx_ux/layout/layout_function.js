...
Jxstar.currentPage = function() {
	...
	*/
	...
	tabFunction.on('tabchange', function(tabPanel, activeTab){
		...
		} else if (pagetype == 'cusgriddes') {
			/**
			 * 显示自定义表格设计器
			 */
			var hdCall = function(f) {
				f.createDesign(selfunid, tabFunction.getComponent(1));
			};
			//异步从JS文件加载功能对象
			Request.loadJS('/zhx/studio/pub/designer_grid_cus.js', hdCall);
		} else
		//显示表单设计器
		if (pagetype == 'formdes') {
			//取创建页面的函数
			var hdCall = function(f) {
				var parent = tabFunction.getComponent(6);
