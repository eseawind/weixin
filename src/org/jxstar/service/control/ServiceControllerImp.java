...
public class ServiceControllerImp implements ServiceController {
	...
	public boolean execute(RequestContext requestContext) {
		...
		//审批事件才有值，指当前审批的功能
		String checkFunId = requestContext.getRequestValue("check_funid");
		...
		} else if (checkFunId.length() > 0 && (eventCode.indexOf("process_") == 0 || eventCode.indexOf("task_") == 0)) {
			lsInvoke = EventDefine.getWfModule(checkFunId, eventCode);
		} else {
		...
		//审批通过后调用audit执行后事件用，修改funID为目标审批功能ID
		if (checkFunId.length() > 0 && 
		   (eventCode.indexOf("process_") == 0 || eventCode.indexOf("task_") == 0)) {
