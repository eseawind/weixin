...
public class AccessTokenUtil {
	...
	static DBServiceBO ds= new DBServiceBO();
	/**
	 * 发起https请求并获取结果
	 * 
	 * @param requestUrl
	...
	public static AccessToken getSingleAccessToken() {
		if (isTokenExpires(accessToken)) {
			Map<String, String> token = ds.getToken(appid, appsecret);
			if (token.isEmpty()) {
				getTokenByHttp(appid,appsecret);
			}else{
				try{
				accessToken.setToken(token.get("token"));
				accessToken.setExpiresIn(Integer.parseInt(token.get("expiresin")));				
				accessToken.setExpiresDate(sdf.parse(token.get("expiresdate")));
	...
	public static void getTokenByHttp(String appid,String appsecret){
		...
		if (null != jsonObject) {
			try {
				...
				Date now = new Date();
				now.setTime(now.getTime()+jsonObject.getInt("expires_in")*1000);
				accessToken.setExpiresDate(now);
				//将token存入数据库
				ds.insertToken(appid,appsecret,token,String.valueOf(expiresin),sdf.format(now));
			} catch (Exception e) {
				accessToken = null;
				// 获取token失败
